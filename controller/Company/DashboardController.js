...
exports.getDashboardDataAPI = async (req, res, next) => {
    try {
        ...
        const unsoldApartment = await Apartment.find({ created_by: userId, status: false })
        ...
        const paidCommanAreaBill = await Bill.find({ status: true, bill_data_type: "common-area-bill", created_by: userId }).populate('apartment_id').populate('bill_type').populate({
            path: "payments",
            model: "Payment"
        });
